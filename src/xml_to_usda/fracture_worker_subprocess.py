...
from .fracture_export_service import FractureExportResult, export_fracture_usda_from_conversion_request
from .fracture_preview_service import FracturePreviewResult, FracturePreviewSettings, generate_fracture_preview_from_conversion_request
...
FRACTURE_WORKER_COMMAND = "fracture-worker"
...
def read_fracture_worker_result(path: str | Path) -> FractureExportResult | FracturePreviewResult | None:
    ...
    with result_path.open("rb") as handle:
        payload = pickle.load(handle)
    if not isinstance(payload, (FractureExportResult, FracturePreviewResult)):
