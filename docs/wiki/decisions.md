...
Scale requested noise uniformly to the first physical terminal, branch, or allowed bend, reserving one lattice edge. Preserve source-triangle provenance for barycentric UV/color/material/skin transfer; caps use planar cutter UVs, boundary-ring attributes, and explicit tags.

Multi-cut stays sequential: oneTBB already parallelizes internally, Python does
not release the GIL, and 1/2/4-process gates lost at 37 independent cuts while
raising RSS. Revisit only after eliminating serialization/startup, with speed,
RSS, exact-result, and packaged-stability gates.

Prepared sessions are scoped to one window/job. Share source analysis,
triangulation, and connectivity; keep closure/provenance/closed Manifold
immutable per cut; noise rebuilds only cutter/results. Reuse only identical
`FractureCutSite` plus cutter settings, never joint name alone. A changed plan
creates a session, although new stump/branch cuts may reuse unchanged results.

Evaluate same-shell cuts in plan order against the current resolved-parent piece;
keep intersections with parent and assign complements to child. Missing parent
region fails loudly. Distinct cutter provenance preserves cap attribution.
Preserve source normals; harden every source/cap perimeter and cap edges at
dihedral >=90 degrees in authored mesh data. Resolve deepest piece ownership by
one parent-chain walk, shared by Preview, Export, and Boolean preparation.

Repeated Part ownership starts from skeleton binding and only ancestor segment
cuts. Flat mode uses physical-bone projection and `segment_t`; nested pivots
climb parent pieces. Detailed mode uses the already-built matching cutter by
barycentric height, deepest first. No second cutter/noise field. No intersection
keeps flat ownership; never borrow a nearby edge or sibling surface.

An automatic cut may claim an immediate-parent face only with positive child-
subtree influence; centroid projection chooses its side. Parent-only and sibling
collars stay parent-owned. Derive eligibility once per face for flat and
Detailed paths. One topology/attribute/collision/part-ownership contract serves
preview and export.
